...
-- Update foreign key constraints to ensure proper cascading deletes (only for existing tables)
DO $$ 
BEGIN
    -- Update testimonials table to use CASCADE DELETE (if table exists)
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'testimonials') THEN
    ...
    -- Update course_enrollments table to use CASCADE DELETE (if table exists)
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'course_enrollments') THEN
    ...
    -- Update payments table to use CASCADE DELETE (if table exists)
    ...
    -- Update member_reviews table to use CASCADE DELETE (if table exists)
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'member_reviews') THEN
    ...
    -- Update referral_analytics table to use CASCADE DELETE (if table exists)
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'referral_analytics') THEN
    ...
    -- Update notification_logs table to use CASCADE DELETE for created_by (if table exists)
    ...
    -- Update notification_templates table to use CASCADE DELETE for created_by (if table exists)
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'notification_templates') THEN
    ...
    -- Update system_settings table to use CASCADE DELETE for updated_by (if table exists)
...
BEGIN
    ...
    -- Check associated tables (only if they exist)
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'payments') THEN
