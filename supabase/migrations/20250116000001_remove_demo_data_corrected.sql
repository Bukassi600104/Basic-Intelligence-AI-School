-- Location: supabase/migrations/20250116000001_remove_demo_data_corrected.sql
-- Purpose: Remove all demo/test data with proper deletion order to avoid foreign key constraints
...
-- 1. First, identify demo user IDs
DO $$
DECLARE
    admin_id UUID;
    instructor_id UUID;
...
BEGIN
    ...
    RAISE NOTICE 'Found demo users - Admin: %, Instructor: %, Student1: %, Student2: %', 
        admin_id, instructor_id, student1_id, student2_id;

    -- 2. Remove system settings that reference demo users (SET NULL the updated_by field)
    UPDATE public.system_settings 
    SET updated_by = NULL 
    WHERE updated_by IN (admin_id, instructor_id, student1_id, student2_id);

    -- 3. Delete testimonials created by demo users
    DELETE FROM public.testimonials 
    WHERE user_id IN (admin_id, instructor_id, student1_id, student2_id);
    ...
    WHERE instructor_id IN (admin_id, instructor_id, student1_id, student2_id);
    ...
    WHERE id IN (admin_id, instructor_id, student1_id, student2_id);
